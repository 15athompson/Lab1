...
void remove_student() {
    ...
    // Move all students after the found index one position back
...
int main() {
    ...
    FILE* test_file = fopen("test_write_permission.txt", "w");
    if (test_file == NULL) {
        perror("Error: Unable to write to the current directory");
        printf("Please ensure you have write permissions for the current directory.\n");
        return 1;
    }
    fclose(test_file);
    remove("test_write_permission.txt");
